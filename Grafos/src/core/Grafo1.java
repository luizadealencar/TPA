...
public class Grafo1{
	...
	public void BFS(DirectedGraph<String, EdgeType> g, HashMap<String, VData> dVertices, String s){
		Queue<String> Q=new LinkedList<>();
	   	Q.clear();
	   	Q.add(s);
	   	VData ud=dVertices.get(s);
	   	ud.cor=1;
	   	ud.td=0;
	   	while(Q.size()!=0){
		   	String aux=Q.element();
		   	Q.poll();
			for(String v : g.getNeighbors(aux)){
				VData vd=dVertices.get(v);
				if(vd.cor==0){
					vd.cor=1;
					vd.td=ud.td+1;
   	...
   	public void visitaBFS(DirectedGraph<String, EdgeType> g) {
		HashMap<String,VData> dVertices=new HashMap<String,VData>();
		for(String u : g.getVertices()){
			VData vd=new VData();
			vd.cor=0;
